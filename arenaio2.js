...
//    }
  ...
  if (!document.querySelector('.grid') && document.querySelector('.gridCanvasOuter')) {
    ...
    for (let s of scripts) {
      if (s.textContent.includes('let cellColors =')) {
        const cellColorsMatch = s.textContent.match(/let cellColors = ({.+?});/s);
        if (cellColorsMatch) {
          const validJsonStr = cellColorsMatch[1].replace(/'/g, '"').replace(/,\s*}/, '}');
          cellColors = JSON.parse(validJsonStr);
        }
        const gridMatch = s.textContent.match(/let GRID_SIZE = (\d+);/);
        if (gridMatch) {
...
Object.entries(equipPresets).forEach(([key, value])=>{
  ...
  async function refreshArenaInfo() {
    ...
    try {
      ...
      let cellColorsString = scriptContent.match(/let cellColors = ({.+?})/s)[1];
      const validJsonStr = cellColorsString.replace(/'/g, '"').replace(/,\s*}/, '}');
      let cellColors = JSON.parse(validJsonStr);
  ...
  async function fetchAreaInfo(refreshAll){
    ...
    cells.forEach(elm => {
      ...
      if(refreshAll || !hasInfo || isRefreshed) {
        ...
        const url = `https://donguri.5ch.io/teambattle?${searchParams.toString()}`;
        fetch(url)
          .then(res =>
