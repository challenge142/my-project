...
(()=>{
//
const DEFAULT_INTERVAL_SECONDS = 5; // 初期値（5秒）

  // 実行間隔をミリ秒で取得する関数
  function getAutoJoinIntervalMs() {
    const input = document.getElementById('aat-interval-input');
    // 入力値を取得。無効な値や1未満の場合は、初期値を使用し、最低1秒を保証
    const seconds = parseInt(input ? input.value : DEFAULT_INTERVAL_SECONDS, 10);
...
// 実行間隔入力フィールドの作成
    ...
    // 配置を中央寄せにし、上部にマージンを設定
    intervalContainer.style.cssText = 'display: flex; align-items: center; justify-content: center; margin-top: 5px;';

    const intervalLabel = document.createElement('span');
    intervalLabel.textContent = '実行間隔 (秒):';
    intervalLabel.style.cssText = 'font-size: 65%; color: black; margin-right: 5px;';

    const intervalInput = document.createElement('input');
    intervalInput.id = 'aat-interval-input'; // 間隔取得関数から参照するためのID
    intervalInput.type = 'number';
    intervalInput.min = '1';
    intervalInput.value = DEFAULT_INTERVAL_SECONDS;
    intervalInput.style.cssText = 'width: 40px; font-size: 100%; padding: 2px; text-align: center; background-color: #eee; color: black; border: 1px solid #ccc;';

    intervalContainer.appendChild(intervalLabel);
    intervalContainer.appendChild(intervalInput);
    // ツールバーの末尾に追加します
    toolbar.appendChild(intervalContainer);
//
...
//    }
  ...
  (()=>{
    ...
    equipSwitchButton.textContent = '$25B6武器';
  ...
  function addCustomColor() {
    ...
    editEndButton.textContent = '$2713';
    ...
    for (const region of regions[cellType]) {
      ...
      try {
        ...
        if (text.startsWith('アリーナチャレンジ開始')
          ...
          success = true;
          message = '[成功] ' + lastLine;
          // processType = 'return'; // returnの削除に伴い不要
        ...
        if (processType === 'return') {
          ...
          logMessage(region.join(','), message, next);
          break; // 成功したら次の実行に移るため for ループを抜ける
        } else if (processType === 'continue') {
          continue;
        } else if (processType === 'break') {
          break;
        }

        // await sleep(sleepTime * 1000); // 長時間待機の削除
      } catch (e) {
        if (e.message === '再ログインしてください') {
          logMessage(region, '[停止] どんぐりが見つかりませんでした', '');
          // isAutoJoinRunning = false; // 削除済み
          clearInterval(autoJoinIntervalId);
          // return; // 削除済み
        } else if (e === 403) {
...
if (isAutoJoinRunning) {
    autoJoinIntervalId = setTimeout(autoJoin, getAutoJoinIntervalMs());
  }
