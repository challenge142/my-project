...
(function() {
  ...
  const CSS = `
    ...
    #dba-br-float {
      ...
      max-height: min(78svh, calc(100svh - 24px));
      border: 4px solid #080;
      border-radius: 12px;
      padding: 0;
    ...
    #dba-br-float .dba-modal__mid {
      overflow: auto;
      max-height: calc(min(88svh, calc(78svh - 24px)) - 110px);
      scrollbar-gutter: stable both-edges;
    }
...
/* 右側に寄せるための設定を追加 */
      ...
      margin-left: auto;
    /* --- 追記 --- */
  position: fixed; /* モーダル解除に伴い位置を固定 */
  top: 50px;      /* ファンクションバーの下に配置 */
  bottom: 20px;      /* ファンクションバーの下に配置 */
  right: 6px;
  z-index: 10000;  /* ゲーム画面より上に表示 */
  pointer-events: auto; /* ウィンドウ内は操作可能にする */
      overflow: hidden;
    }
  ...
  function normalizeAutoEquipCandidateKey(key){
    ...
    const m = s.match(/^(N|R|SR|SSR|UR)(警エ|警|エ)?$/u);
    if(!m) return s0;
    return `${m[1]}${m[2] || ''}`;
  }
  ...
  function openBattleResultOptionsModal(){
    ...
    try{ dlg.show(); }catch(_e){ dlg.setAttribute('open',''); }
  }

  // ★重要：戦闘結果のオプション modal は常に最前面（top-layerの一番上）に保つ
  // - battle-result 側を show で出し直す等で前後関係が崩れるケースの対策
  ...
  function bringBattleResultOptionsModalToFront(){
    ...
    try{ opt.show(); }catch(_e2){ opt.setAttribute('open',''); }
  }
  ...
  function openRosterResultModalWithNode(nodeOrText, titleText){
    ...
    try{ dlg.show(); }catch(_e){ dlg.setAttribute('open',''); }
// showModal() ではなく setAttribute('open','') を使うことで、
// ブラウザによる「画面中央への強制配置」を回避し、CSSの指定を優先させます。
...
//            openRosterResultModalWithNode(`装備切替完了\n${nm}`, '装備ロスター');
  ...
  function openRosterModal(){
    const openNow = () => {
      ...
      if(!dlg2) return;
      try{ dlg2.show(); }catch(_e){ dlg2.setAttribute('open',''); }
    };
...
function avatarsKeyToMap(avatarsKey){
  ...
  function buildSettingsModal(){
    ...
    function askCloseIfDirty(){
      ...
      try{ adlg.show(); }catch(_e){ adlg.setAttribute('open',''); }
    }
